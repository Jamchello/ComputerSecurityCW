import hashlib
#Agreed parameters for DH - MODP group 14.
n = 

priv_key = int(input("Input your private key: "))
pub_key = int(input("Input your partners public key: "))
#Computing shared secret value
shared_secret = pow(pub_key,priv_key,n)
#Using SHA-256 to create a 256-bit
shared_key = hashlib.sha256(str(shared_secret).encode()).hexdigest()  
print(f'Shared key: {shared_key}')